...
func main() {
	...
	rizzService := services.NewRizzService(database.DB, cfg)
	...
	rizzHandler := handlers.NewRizzHandler(rizzService)
	...
	// Routes
	routes.Setup(app, cfg, authHandler, healthHandler, webhookHandler, moderationHandler, rizzHandler)
